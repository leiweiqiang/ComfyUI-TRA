...
class TclYoloV9Segmentation:
    ...
    def INPUT_TYPES(s):
        return {
            'required': {
                'image': ('IMAGE', ),
                'model_name': (folder_paths.get_filename_list(YOLO_FOLDER), ),
    ...
    def segment(self, image, model_name, class_name):
        ...
        model_path = os.path.join(folder_paths.models_dir, YOLO_FOLDER, model_name)
